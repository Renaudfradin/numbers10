...
function cs_portfolio_init() {
  ...
  if ( ! x_get_option( 'x_portfolio_enable' ) ) {
    return;
  }

  $slug      = sanitize_title( get_option( 'x_custom_portfolio_slug', 'portfolio-item' ) );
...
function cs_portfolio_filters() {
  ...
  $stack           = function_exists('x_get_stack') ? x_get_stack() : 'integrity';
  ...
  if ( $stack == 'integrity' ) {
    $button_content = '<i class="x-icon-sort" data-x-icon-s="&#xf0dc;"></i> <span>' . get_option( 'x_integrity_portfolio_archive_sort_button_text', __( 'Sort Portfolio', '__x__' ) ) . '</span>';
  } elseif ( $stack == 'ethos' ) {
...
function cs_portfolio_item_project_link() {
  ...
  $launch_title  = get_option( 'x_portfolio_launch_project_title', __( 'See it Live!', '__x__' ) );
  $launch_button = get_option( 'x_portfolio_launch_project_button_text', __( 'Launch Project', '__x__' ) );
...
function cs_portfolio_item_tags() {
  ...
  $stack     = function_exists('x_get_stack') ? x_get_stack() : 'integrity';
  $tag_title = get_option( 'x_portfolio_tag_title', __( 'Skills', '__x__' ) );
...
function cs_portfolio_item_social() {
  ...
  $share_project_title = get_option( 'x_portfolio_share_project_title', __( 'Share this Project', '__x__' ) );
  $enable_facebook     = get_option( 'x_portfolio_enable_facebook_sharing', '1' );
  $enable_twitter      = get_option( 'x_portfolio_enable_twitter_sharing'. '1' );
  $enable_google_plus  = get_option( 'x_portfolio_enable_google_plus_sharing', '' );
  $enable_linkedin     = get_option( 'x_portfolio_enable_linkedin_sharing', '' );
  $enable_pinterest    = get_option( 'x_portfolio_enable_pinterest_sharing', '' );
  $enable_reddit       = get_option( 'x_portfolio_enable_reddit_sharing', '' );
  $enable_email        = get_option( 'x_portfolio_enable_email_sharing', '' );
...
function cs_portfolio_page_template_precedence( $request ) {
  ...
  if ( array_key_exists( 'post_type', $request ) && 'x-portfolio' == $request['post_type'] ) {
    if ( get_option( 'x_custom_portfolio_slug', 'portfolio-item' ) === $wp->request && get_page_by_path( $wp->request ) ) {
...
function cs_theme_options_set_transients_before_save() {
  set_transient( 'cs_portfolio_slug_before', get_option( 'x_custom_portfolio_slug', 'portfolio-item' ), 60 );
}
...
function cs_theme_options_set_transients_after_save() {
  set_transient( 'cs_portfolio_slug_after', get_option( 'x_custom_portfolio_slug', 'portfolio-item' ), 60 );
}
