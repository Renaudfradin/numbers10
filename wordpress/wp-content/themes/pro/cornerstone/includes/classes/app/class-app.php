...
class Cornerstone_App extends Cornerstone_Plugin_Component {
  ...
  public function register_app_scripts( $settings, $isPreview = false ) {
    ...
    $app_asset = $this->plugin->versioned_url( 'assets/dist/js/app', 'js' );
    ...
    $worker_asset = $this->plugin->versioned_url( 'assets/dist/js/worker', 'js' );
  ...
  public function get_app_data() {
    ...
    return array(
      'experimental'              => apply_filters('tco_experimental', false),
      'rootURL'                   => $rootUrl,
      ...
      'siteConfirmMessage'        => $this->get_site_install_confirm_message(),
      ...
      'shared' => [
        ...
        'coalescenceSelectorPrefixes' => apply_filters('cs_coalescence_selector_prefixes', ['#cs-content', '#cs-footer']),
        'rowPresets'                  => array(
          '100%',
          '50% 50%',
          '33.33% 33.33% 33.33%',
        ...
        'elements'      => $this->plugin->component( 'Element_Manager' )->get_elements(),
        'elementIcons'  => $this->plugin->component( 'Element_Manager' )->get_element_icons(), // 8kb
      ]
    );
  }
  ...
  public function get_late_data() {
    return array(
      ...
      'fontData'              => apply_filters( 'cs_font_data', $this->plugin->config_group( 'common/font-data' ) ), // 23kb
      ...
      'prefabElements'        => $this->plugin->component( 'Prefab_Elements' )->get_prefabs(), // 8kb
      'defaultPresets'        => CS()->component('Template_Manager')->lookup_default_presets(), // 0kb

      'elementsInspectorData' => $this->plugin->component( 'Element_Manager' )->get_element_inspector_data(), // 354kb
  ...
  public function get_site_install_confirm_message() {

    $extra = '';

    if ( ! class_exists( 'RevSlider' ) ) {
      $extra = __( 'Note: Revolution Slider is not active. Activate it from the plugins page, as some Sites can import sliders.', 'cornerstone' );
      $extra .= '<br/><br/>';
    }

    return sprintf( __( '%sInstalling a site (demo content) will not alter any of your pages or posts, but it will overwrite your Theme Options. This is not reversible unless you have previously made a backup of your settings. Are you sure you want to proceed?', 'cornerstone' ), $extra );
