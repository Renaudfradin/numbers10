...
function x_atts() {
  ...
  // Combine all sets of arrays while respecting the class attribute
  // Try to decode json sets
  foreach($args as $set) {
    if (is_string($set)) {
    ...
    if (isset($merged['class']) && isset($set['class']) ) {
      $set['class'] = trim($merged['class']) . ' ' . trim($set['class']);
    }
  ...
  foreach ( $merged as $attr => $value ) {
    if ( is_null( $value ) ) {
      $result .= esc_attr($attr) . ' ';
    } else {
      $result .= esc_attr($attr) . '="' . esc_attr( $value ) . '" ';
    }
  }
...
// X i18n Lookup
// =============================================================================
// 01. Add 'noopener noreferrer' to a string if it doesn't exist yet.
